...
document.write('<script src="https://www.189.cn/client/wap/common/js/aes.js"></script>');
...
*	
*	@Parameters
*		extObj：				20190603修改，原参数名functionCode，更改为extObj，作为可扩展参数，该参数兼容以前只传functionCode的场景
*							1、extObj：'xxx'												只传functionCode场景
*							2、extObj：{'functionCode' : 'xxx', 'ajaxUrl' : 'xxx'}		对象场景
*		data：				参数		
...
*
*/
function doAjax(extObj, data, successCallback, failedCallback, errorCallback, isShow, isAsynchronous,callBack,pendings){
	var params = {};			//参数对象
	if(typeof extObj === 'string'){
		params.functionCode = extObj;
	}else if(typeof extObj === 'object' && extObj.constructor === Object){
		$.extend(params, extObj);
	}
	...
	//isEncrypt 默认不加密 lrj 20181228
	var activityType = sessionId = '', url='',outTime=60000, obj = {},isEncrypt=false;
	...
	if(isNull(sessionStorage.toolsObj)){
		...
		isEncrypt = (obj.isEncrypt == true || obj.isEncrypt == 'true') ? true : false;
	}
	...
	var param = '{'+
				...
				'"requestContent":' +
					data +
			'}';
	//功能js toolsObj里添加isEncrypt 默认不加密，为true时加密 lrj 20181228
	if(isEncrypt) {
		param = encrypt(param);
	}
...
function isTelecomPhone(obj) {
    ...
    //号段改为接口判断本异网的需求，前端不再判断 by lil on 2019/11/07
    if (obj.length === 13 && obj.substring(0, 5) === '10649') {
        return true;
    } else {
        return isPhoneNumber(obj);
    }
}
	...
	randomBtn = function(options){
		...
		**/
		this.reset = function(){
			...
			_rthis.settings.target.html(_rthis.settings.html).removeClass(_rthis.settings.reClass);
					...
					if(isNull(_rthis.settings.funCode)){
						...
						doAjax(_rthis.settings.funCode,_rthis.settings.param,function(data){
							...
							//调用后台失败 重新获取
								...
								btnHelp.randomLock = false;
								//_this.html(_rthis.settings.reHtml);
								//获取短信验证码前，先校验图形验证码 lrj 20190408
								//W_0016、W_0017为图形校验码错误，按钮文案为"获取验证码"，其它错误码重置按钮文案为"重新获取"
								if(['W_0016','W_0017'].indexOf(data.headerInfo.code) < 0){
									_this.html(_rthis.settings.reHtml);
								}
							}
						},function(data){
...
//是否在QQ浏览器内打开 lrj 20181218
//返回return true:是  false:否
function isQQ(){
	var match = navigator.userAgent.match(/(?:\bV1_AND_SQI?_|QQ\/)([\d\.]+)/); //qq浏览器
	if(isNull(match)){
		return false;
	}else{
		return true;
...
//打开欢go客户端,页面2S后跳转指定链接 lrj 20181218
//targetUrl页面跳转链接
function openApp(targetUrl) {
	var appSrc = ""; //下载地址
	if (getPhoneType() == 2) { //安卓
		appSrc = 'ctclient://startapp/android/open'; //打开欢购客户端
		if (isQQ() || isWeiXin()) {
			appSrc = 'https://a.app.qq.com/o/simple.jsp?pkgname=com.ct.client&android_schema=' + encodeURIComponent(appSrc);
		}
		locationPage(appSrc, true);
	} else if (getPhoneType() == 1) { //iOS
		appSrc = "ctclient://startapp/ios/open"; //打开欢购客户端
		if (isQQ() || isWeiXin()) {
			appSrc = 'https://a.app.qq.com/o/simple.jsp?pkgname=com.ct.client&ios_scheme=' + encodeURIComponent(appSrc);
		}
		locationPage(appSrc, true);
	}
	//跳转链接为空时不跳转
	...
	var t = Date.now();
	setTimeout(function() { //超过两秒没打开客户端，打开下载页
		if (Date.now() - t < 2100) {
			locationPage(targetUrl, true); //跳下载页
		}
	}, 2000);
}

/**
 * 是否在支付宝内打开 20190102 lrj
 * return true: 支付宝内 ，false: 支付宝外
 */
function isZFB(){
	var ua = window.navigator.userAgent.toLowerCase();
...
/*
 * QQ内置浏览器判断 lrj 20190411
 */
function qqBrowser(){
	var u = navigator.userAgent.toLowerCase();
    var isAndroid = u.indexOf('android') > -1 && u.match(/qq\/[0-9]/i); //android终端QQ内置浏览器
    var isIos = u.indexOf('mac os x') > -1 && u.match(/qq\/[0-9]/i); //ios终端QQ内置浏览器
    if (isAndroid) {
        return "androidQQ"; //android中QQ内置浏览器
    } else if (isIos) {
        return "iosQQ"; //ios中QQ内置浏览器
    } else {
        return ""; // 其它浏览器
    }
}
...
/*
** 调用微信小程序支付 lrj 20190516
** 需要引用https://www.189.cn/client/wap/common/js/jweixin-1.3.3.js
** orderId ：订单号, backUrl:支付回调地址，failureTime:订单失效时间
 */
function miniProgramPay(orderId,backUrl,failureTime){
	var path = '/pages/unlimitedPackages/pay/pay?orderid='+encodeURIComponent(encrypt(orderId))+'&backUrl='+encodeURIComponent(backUrl)+'&failureTime='+failureTime;
	wx.miniProgram.navigateTo({ url: path });	//微信小程序跳转 支付
}

//域名白名单校验 lrj 20190527
//backUrl: 域名需校验链接
function backUrlCheck(backUrl){
	if(isNull(backUrl)) {return false;}
	var url = decodeURIComponent(backUrl);//url解码
    var host = /^(http|https):\/\//; //校验协议https://, http://
    var testHostList = /^(clientservice.189.cn:)(911[0-9]{1}|990[0-5]{1})$/;//准一域名白名单
    var hostList = ['wappay.189.cn','www.189.cn','wapzt.189.cn',
                    'a.189.cn','cservice.client.189.cn:9092','wapclient.189.cn:9092',
                    'lmk.189.cn:9093','lm.189.cn','wapzt.189.cn:8010',
                    'wapzt.189.cn:8011','wapzt.189.cn:8080','q.189.cn','kid.189.cn:5443'];//生产域名白名单
	if(host.test(url)) {
        //截取域名http[s]://xxxxx/
        var tempHost = url.substring(url.indexOf('//')+2); //   xxxxx/
        var tempIndex = tempHost.indexOf('/'); // xxxxx/ 索引
        //截取域名+端口号
        var curHost = tempHost.substring(0,(tempIndex == -1?tempHost.length:tempIndex));
        //不在准一和生产域名白名单内跳404页面
	    if(!testHostList.test(curHost) && hostList.indexOf(curHost) == -1){
        	jumpTo404(); //跳404
        }
    } else {
        jumpTo404(); //跳404
...
//跳转到公共404页面 lrj 20190527
function jumpTo404(){
	window.location.href = 'https://www.189.cn/client/wap/common/page/error_404.html?errorCode=20001';
    return false;
}
//通过manId获取客服务链接 manId必传(字符串)
function getImLink(manId){
	var imEncodeUrl = '';
	doAjax({'ajaxUrl' : '/wapclient/kefuImEncode.do', 'functionCode' : 'kefuImEncode'}, {'manId' : manId}, function(data){
		if(data.headerInfo.code === 'W_0000' && data.responseContent.serviceCode === '0'){
			imEncodeUrl = data.responseContent.imEncodeUrl;
		}
	}, function(){
	}, function(){
	}, true, false);
...
//跳转客服务链接
function goIm(manId){
	var link = getImLink(manId);
    if(link){
    	location.href = link;
    }
}

//添加客服icon入口 调用形式：kefuIcon({'imLink' : xxx});
function kefuIcon(extObj){
    if(kefuIcon.single) return;			//防止多次添加
    var obj = {
			cls: '',  //客服类名
            zindex : 0,
            top    : '60%',
            ...
            width  : '15%',//需传百度比格式
            imLink : '',
            url    : 'https://www.189.cn/client/wap/common/images/kefu_icon.png',//客服图标
            preFun : function(){}		//跳转链接前注册方法
        },
        $kefuIcon;
    $.extend(obj, extObj);
    obj.right = obj.right.replace("%","")/100; //百分比转小数
    if(obj.left){
        obj.left = obj.left.replace("%","")/100;
    }
    $kefuIcon = $('<div class="' + obj.cls + '" style="z-index:' + obj.zindex + '; position:fixed; max-width:'+ ((+obj.width.replace("%",""))/100 * 720) +'px; width:'+ obj.width +'; top:'+ obj.top +';"><img width="100%" src="'+ obj.url +'" /></div>').click(function(){
        obj.preFun();
        location.href = obj.imLink;
    });
    $('body').append($kefuIcon);
    reset();
    window.addEventListener('resize', reset);
    function reset(){
        var w = $(window).width();
        var position = null;
        var direction = obj.left ? 'left' : 'right';
        if(w > 720){
            position = ((w - 720) / 2) + obj[direction] * 720;
        }else{
            position = obj[direction] * w;
        }
        $kefuIcon.css(direction, position + "px");
    }
    kefuIcon.single = true;				//防止多次添加
};
/*
 * 是否满足电信手机号码的要求
 * @Parameters
 ...
 * failedCallback：	失败回调函数
 * errorCallback：	异常回调函数
 * state 校验状态id   -1 接口失败   0 异网手机号  1本网手机号
 * 
 * */ 
function isTelecomPhoneNum(obj) {
    var state='',options = {
    	phoneNumber: "",//需校验手机号码
    	sessionid: '',//sessionid
    	successCallback: function(data){},//成功回调函数 
    	failedCallback: function(data){}, //失败回调函数
    	errorCallback: function(){}	  //异常回调函数
    };
    $.extend(options, obj);
    var param = {
    ...
    doAjax('judgeTelecomPhone', param, function (data) {
        if (data.headerInfo.code === 'W_0000' && data.responseContent.serviceCode === '0') {
            if (data.responseContent.operators == '1') {
            	state = data.state = 1;
            }else{
            	state = data.state = 0;
            }
        	...
        	state = data.state = -1;
        	options.failedCallback(data);
        }
    }, function () {//异常
    	var data = {state: (state = -1),message: "系统繁忙，请稍后再试！"};
...
function clipboard(extObj){
    var obj = {
        ...
        creatEl = null;                 //复制文本内容时创建的文本框，使用完即删除
    $.extend(obj, extObj);
    if(obj.text){
        selectText(obj.text);
    }else if(obj.el){
    ...
    function selectText(text){
        var pageDir = "rtl" === document.documentElement.getAttribute("dir");
        creatEl = document.createElement("textarea");
        creatEl.value = text;
        creatEl.style.position = "absolute";
        creatEl.style[pageDir ? "right" : "left"] = "-9999px";
        document.body.appendChild(creatEl);
        selectEle(creatEl);
    }
    //选中元素内容并复制内容
    function selectEle(el){
        if(!el) throw new Error("无效选择器");
        if ("SELECT" === el.nodeName)
            el.focus();
        else if ("INPUT" === el.nodeName || "TEXTAREA" === el.nodeName) {
            var n = el.hasAttribute("readonly");
            n || el.setAttribute("readonly", "");
            el.select();
            el.setSelectionRange(0, el.value.length);
            n || el.removeAttribute("readonly");
        } else {
    ...
    //将选中的文本复制到剪切版
    //flag备注：false：复制失败，true：复制成功，undefined：未知(安卓UC浏览器等测试结果复制成功)
    function copyText(){
        var flag = false;
        try { flag = document.execCommand(obj.action); } catch (el) {}
        creatEl && document.body.removeChild(creatEl);
        obj.callback(flag);
    }
}
(function($){
	 ...
	 * @Parameters
	 * $el：	弹出框对象
	 * 备注：需为遮罩层添加commonMask类，弹框内允许滚动的元素添加commonScroll类
	 * */ 
    window.openBefore = function($el){
        if($el){
            $el.scrollTop(0);						//弹框滚动条置顶
            $el.find('.commonScroll').scrollTop(0);	//内部滚动区域置顶
            overscroll($el);
            overscroll($el.find('.commonScroll'));
        }
        $('body').css('overflow','hidden');			//禁止滚动条滚动
    };
    //页面开启滚动
    window.closeAfter = function(){
        $('body').css('overflow','auto');
    };
    //内部方法，不允许外部调用
    var overscroll = function($el){
        if(!$el) return false;
        $el.off('touchstart').off('touchmove').on('touchstart', function(e){
            var scrollTop = this.scrollTop,
                offsetHei = this.offsetHeight,
                scrollHei = this.scrollHeight;
            if(scrollTop === 0){
                this.scrollTop = 1;
            } else if(scrollTop + offsetHei >= scrollHei){
                this.scrollTop = scrollTop - 1;
            }
            this.pageY = e.originalEvent.targetTouches[0].pageY;
            e.stopPropagation();
        }).on('touchmove', function(e){
            var pageY = e.originalEvent.targetTouches[0].pageY;
            if((this.scrollTop <= 1) && (pageY > this.pageY)){
                e.preventDefault();
            }else if((this.scrollTop + 1 + this.offsetHeight >= this.scrollHeight) && (pageY < this.pageY)){
                e.preventDefault();
            }
            e.stopPropagation();
        });
    ...
    $(function(){
        //遮罩层禁止touchmove事件
        $('.commonMask').off('touchmove').on('touchmove',function(e){
            return false;
        });
    });
})(jQuery);