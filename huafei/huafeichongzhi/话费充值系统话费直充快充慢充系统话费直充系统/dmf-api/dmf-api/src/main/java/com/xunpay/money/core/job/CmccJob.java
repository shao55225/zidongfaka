...
import com.xunpay.money.utils.CmccTest;

public class CmccJob extends BaseJob {
    private static final Logger logger = Logger.getLogger(CmccJob.class);
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        if (companyOrderList != null && companyOrderList.size() > 0) {
            for (CompanyOrder companyOrder : companyOrderList) {
                ...
                int flag=2;
               	if(url!=null) {
               		CmccTest test=new CmccTest();
               		flag = test.orderStatus(url);
