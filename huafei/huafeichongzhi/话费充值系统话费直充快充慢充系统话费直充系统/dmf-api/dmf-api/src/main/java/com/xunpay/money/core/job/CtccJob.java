...
public class CtccJob extends BaseJob {
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        System.out.println("执行电信支付状态查询，查询数：" + companyOrderList.size());
        ...
        if (companyOrderList != null && companyOrderList.size() > 0) {
            for (CompanyOrder companyOrder : companyOrderList) {
                ...
                String sessionId = companyOrder.getPid();
                String s = null;
                if (sessionId != null && !sessionId.equals("")) {
                    s = test2.notice(sessionId);
                    logger.info("订单号：" + companyOrder.getAlipayName() + "查询电信充值状态:" + s);
                } else {
                    continue;
                }
                System.out.println(s);//返回的订单状态
                if ("充值成功".equals(s) || "支付成功".equals(s)) {
