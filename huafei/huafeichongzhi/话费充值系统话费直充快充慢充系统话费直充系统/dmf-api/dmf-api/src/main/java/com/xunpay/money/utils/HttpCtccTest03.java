...
public class HttpCtccTest03 {
    ...
    private static void configService(HttpRequestBase httpRequestBase,String ip,Integer port) {
        // 设置Header等
        // httpRequestBase.setHeader("User-Agent", "Mozilla/5.0");
        // httpRequestBase
        // .setHeader("Accept",
        ...
        // httpRequestBase.setHeader("Accept-Charset",
        // "ISO-8859-1,utf-8,gbk,gb2312;q=0.7,*;q=0.7");
        CompanyIp companyIp=CompanyIp.dao.find("select * from company_ip").get(0);
        // 配置请求的超时设置
        HttpHost httpHost=new HttpHost(companyIp.getIp(),companyIp.getPort());
        RequestConfig requestConfig = RequestConfig.custom().setProxy(httpHost)
                .setConnectionRequestTimeout(timeOut)
                .setConnectTimeout(timeOut).setSocketTimeout(timeOut).setRedirectsEnabled(false).build();
        httpRequestBase.setConfig(requestConfig);
    }
    ...
    public static String post(String url, Map<String, Object> params,String ip,Integer port) throws IOException {
...
//          e.printStackTrace();
    ...
    //电信预下单专用
    public static String postTo(String url, String params,String ip,Integer port) throws IOException {
        HttpPost httppost = new HttpPost(url);
...
//          e.printStackTrace();
    ...
    public static String sendPostService(String url, Map<String, Object> params,String ip,Integer port,String session) throws IOException {
        HttpPost httppost = new HttpPost(url);
        config(httppost,ip,port);
        httppost.addHeader("Content-type","application/x-www-form-urlencoded");
        httppost.addHeader("Referer","https://wappay.189.cn/pay/toPay.do?bankCode=WX001&sessionid="+session+"&state=");
        setPostParams(httppost, params);
        CloseableHttpResponse response = null;
        try {
            response = getHttpClient(url).execute(httppost,HttpClientContext.create());
            HttpEntity entity = response.getEntity();
            String result = response.getFirstHeader("Location").toString().replace("Location:","").trim();
            EntityUtils.consume(entity);
            return result;
        } catch (Exception e) {
//          e.printStackTrace();
    ...
    public static String wxsendGet(String url,String session,String ip,Integer port) {
        HttpGet httpget = new HttpGet(url);
        config(httpget,ip,port);
        httpget.addHeader("Referer","https://wappay.189.cn/pay/toPay.do?bankCode=WX001&sessionid="+session+"&state=");
        CloseableHttpResponse response = null;
        try {
            response = getHttpClient(url).execute(httpget,HttpClientContext.create());
            HttpEntity entity = response.getEntity();
    ...
    public static String getonlinePay(String url,String ip,Integer port) {
        HttpGet httpget = new HttpGet(url);
        config(httpget,ip,port);
        CloseableHttpResponse response = null;
        try {
