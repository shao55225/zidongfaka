...
<head>
  ...
  function queryPid() {
	  $.get("${ctx}/order/createQr", function(data){
  ...
  function queryOrder(orderNo) {
	  $.get("${ctx}/order/queryOrder?orderNo=" + orderNo, function(data) {
		  if (data.result == "success") {
	...
	<div class="am-g">
		<div class="am-u-lg-2">
			<div class="am-input-group am-input-group-secondary am-form-group">
				<a href="${ctx}/order/addSettleItem"><span class="am-input-group-label">添加收款支付宝</span></a>
			</div>
		</div>
		<div class="am-u-lg-2">
		...
		<div class="am-u-lg-7">
			<div class="am-input-group am-input-group-secondary am-form-group">
				<strong style="color:red;">未设置收款支付宝时，会自动转入到【${defaultPid}】</strong>
			</div>
		</div>
		<div class="am-u-lg-1">
			...
			<table class="am-table am-table-striped am-table-hover am-table-striped am-text-nowrap">
				...
				<tbody>
					...
					<tr ${s.status == '禁用' ? 'style="color:#dd514c;font-weight:bold;"' : ''}>
						...
						<td>${s.remark}</td>
						<td>
							<a href="${ctx}/order/editSettleItem/${s.id}" class="am-btn am-btn-secondary am-btn-xs">编辑</a>
							<a href="javascript:deleteItem('${ctx}/order/deleteSettleItem/${s.id}');" class="am-btn am-btn-danger am-btn-xs">删除</a>
							
							<c:if test="${s.status == '正常'}">
							<a href="${ctx}/order/downSettleItem/${s.id}" class="am-btn am-btn-warning am-btn-xs">禁用</a>
							</c:if>
							
							<c:if test="${s.status == '禁用'}">
							<a href="${ctx}/order/upSettleItem/${s.id}" class="am-btn am-btn-success am-btn-xs">启用</a>
							</c:if>
						</td>
					</tr>
					</c:forEach>
