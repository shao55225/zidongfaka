...
<head>
    ...
    <script type="text/javascript">
        ...
        function comfirmOrder(url) {
            if (url != undefined) {
                //联通专用
                //var comUrl=encodeURIComponent(url);
                //  var str=comUrl.split("payStr%3D");
                //  var urlOpen="https://unipay.10010.com/udpNewPortal/miniwappayment/weixinH5Return?payStr="+str[1]
                window.open(url); //电信专用
            }
        }
        ...
        function repayOrder(id) {
            ...
            var passWord = prompt("请输入超级管理员密码：（此操作会将支付中订单及充值中变成支付成功）");
            ...
            $.get("${ctx}/order/repayOrder/" + id, function (data) {
        ...
        function noticeItem(id) {
            $.get("${ctx}/order/noticeOrder/" + id, function (data) {
                if (data == "success") {
                    alert("操作成功！");
                    location.reload();
                }
                ...
                <div class="am-input-group am-input-group-secondary am-form-group">
                    ...
                    <select name="company_id" id="companyId" class="am-form-field">
                        ...
                        <c:forEach items="${companys}" var="c">
                            <option value="${c.id}" ${company_id == c.id ? "selected" : ""}>${c.nickname}
                                - ${c.username}</option>
                        </c:forEach>
                    </select>
                </div>
            ...
            <div class="am-u-lg-4">
                <div class="am-input-group am-input-group-secondary am-form-group">
                    ...
                    <select name="status" style="float:left;width:100px;">
                        ...
                        <option value="支付超时" ${status == '支付超时' ? 'selected' : ''}>支付超时</option>
                    </select>
            ...
            <table class="am-table am-table-striped am-table-hover am-table-striped am-text-nowrap">
                ...
                <c:forEach items="${page.list}" var="o" varStatus="i">
                    <tr ${u.status == '已拒绝' ? 'style="color:red;"' : ''}>
                        ...
                        <!-- <td>${o.token}</td>  am-btn-warning  am-btn-danger -->
                        ...
                        <td>
                            ...
                            <a class="am-btn am-btn-secondary" href="javascript:viewOrder(${o.id});">详情</a>
                            <c:if test="${not empty o.return_url}">
                                <a class="am-btn am-btn-success"
                                   href="javascript:comfirmOrder('${o.return_url}');">查单</a>

                                <c:if test="${o.status == '支付中'}">
                                    <a class=" am-btn-danger" href="javascript:repayOrder('${o.id}');">补单</a>
                                </c:if>
                                <c:if test="${o.company_name !=null}">
                                    <c:if test="${o.status == '支付失败'}">
                                        <a class=" am-btn-danger" href="javascript:repayOrder('${o.id}');">补单</a>
                                    </c:if>
                                </c:if>
                ...
                <c:if test="${page.totalPage > 1}">
                    <div class="am-fr">
                        <ul class="am-pagination ">
                            ...
                            <c:forEach var="i" begin="${page.pageNumber - 7 < 1 ? 1 : page.pageNumber - 7}"
                                       end="${page.pageNumber + 7 > page.totalPage ? page.totalPage : page.pageNumber + 7}">
                                <c:if test="${page.pageNumber == i}">
                                    <li class="am-active"><a class="am-active" href="javascript:void(0);">${i}</a></li>
                                </c:if>
                                <c:if test="${page.pageNumber != i}">
                                    <li><a href="javascript:gridPage(${i});">${i}</a></li>
                                </c:if>
                            </c:forEach>
                            <li class="am-pagination-last"><a
                                    href="javascript:gridPage(${page.totalPage});">&gt;&gt;</a></li>
                        </ul>
                    </div>
                </c:if>
            </div>
